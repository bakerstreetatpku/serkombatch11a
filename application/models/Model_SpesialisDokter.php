<?php
  class Model_SpesialisDokter extends CI_Model {
  
  	var $table = 'srk_spesialisdokter';
	var $column_order = array('sdk_dok_id'); //set column field database for datatable orderable
	var $column_search = array('sdk_dok_id', "sdk_sps_id"); //set column field database for datatable searchable just firstname , lastname , address are searchable
	var $order = array('sdk_dok_id' => 'asc'); // default order  	private $db_sts;
	...
	{	$this->db->from("srk_jabatan");
		$this->db->where("jab_jenis","SpesialisDokter");
		$query = $this->db->get();
	...
	public function get_spesialisdokter($id)
	{	$this->db->from("srk_spesialisdokter");
		$this->db->where("sdk_id", $id);
		$query = $this->db->get();
