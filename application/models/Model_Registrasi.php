<?php
  class Model_Registrasi extends CI_Model {
  	...
  	var $table = 'srk_registrasi';
	var $column_order = array('reg_dok_id'); //set column field database for datatable orderable
	var $column_search = array('reg_dok_id', "reg_sps_id"); //set column field database for datatable searchable just firstname , lastname , address are searchable
	var $order = array('reg_dok_id' => 'asc'); // default order  	private $db_sts;
	...
	public function carispesialis($id)
	{	$this->db->from("srk_spesialis");
	...
	public function geturut($tgl, $ply_id, $dok_id)
	{	$this->db->select("reg_urut");
		$this->db->from("srk_registrasi");
		$this->db->where("reg_dok_id", $dok_id);
		$this->db->where("reg_ply_id", $ply_id);
		$this->db->where("reg_tgl", $tgl);
		$this->db->order_by("reg_urut", "desc");
		$query = $this->db->get();
		$q = $query->row();
	...
	{	$this->db->from("srk_jabatan");
		$this->db->where("jab_jenis","Registrasi");
		$query = $this->db->get();

		return $query->result();
	}

	public function cari_spesialis($id)
	{	$this->db->from("srk_spesialis");	
